...
SELECT id, user_id, name, description, main_card_id, main_card_type_id, sub_card_id, sub_card_type_id, created_at, updated_at FROM decks
WHERE id = ?
LIMIT 1
`
...
const findDecksByUserId = `-- name: FindDecksByUserId :many
SELECT id, user_id, name, description, main_card_id, main_card_type_id, sub_card_id, sub_card_type_id, created_at, updated_at FROM decks
WHERE user_id = ?
ORDER BY updated_at DESC
`

func (q *Queries) FindDecksByUserId(ctx context.Context, userID string) ([]Deck, error) {
	rows, err := q.db.QueryContext(ctx, findDecksByUserId, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
