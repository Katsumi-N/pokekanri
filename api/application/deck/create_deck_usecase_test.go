...
func (m *mockDeckRepository) FindByUserId(ctx context.Context, userId string) ([]*domainDeck.Deck, error) {
	...
	tests := map[string]struct {
		...
		"success": {
			...
			returnDeck: func() *domainDeck.Deck {
				...
				// デッキを作成
				deck, _ := domainDeck.NewDeck(1, "test-user-id", "テストデッキ", "テスト用のデッキです", mainCard, subCard, deckCards)
				return deck
			}(),
			expectError: false,
			expectedDeckDto: &DeckDto{
